...
*/
   ...
   nextRow: while ($row = mysql_fetch_assoc($result))
   {
       ...
       if ($name != $nameRow['name'])
       {
           ...
           fwrite($AIFile, "DELETE FROM `smart_scripts` WHERE `entryorguid`=@ENTRY;\r\n");
       ...
       $query .= "$phaseMask,$row[event_chance]," . ((($row['event_type'] == 2) || ($row['event_type'] == 3) || ($row['event_type'] == 12) || ($row['event_type'] == 18)) ? 1 : 0) . ",";
       ...
       switch($type)
       {
           case 1:
               ...
               while ($innerRow = mysql_fetch_assoc($innerResult))
               {
                   ...
                   fwrite($textsFile, "(@ENTRY,$groupId," . $id++ . ",\"$innerRow[content_default]\",$innerRow[type],$innerRow[language],100,$innerRow[emote],0,$innerRow[sound],\"$innerRow[comment]\")");
               }
